...
public class DBCPOrderedLastSynchronizationList implements Synchronization {
    ...
    private final List<Synchronization> preSyncs = new ArrayList<Synchronization>();
    private final List<Synchronization> postSyncs = new ArrayList<Synchronization>();
    ...
    public void registerInterposedSynchronization(Synchronization synchronization) throws IllegalStateException, SystemException {
        ...
        if (synchronization.getClass().getName().startsWith("org.apache.commons.dbcp2")) {
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.postSyncs.add - Class: " + synchronization.getClass() + " HashCode: "
                        + synchronization.hashCode() + " toString: " + synchronization);
            }
            postSyncs.add(synchronization);

        } else {
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.preSyncs.add - Class: " + synchronization.getClass() + " HashCode: "
                        + synchronization.hashCode() + " toString: " + synchronization);
            }
            preSyncs.add(synchronization);
        }
    }
    ...
    public void beforeCompletion() {
        ...
        while ((lastIndexProcessed < preSyncs.size())) {
            Synchronization preList = preSyncs.get(lastIndexProcessed);
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.preSyncs.before_completion - Class: " + preList.getClass() + " HashCode: "
                        + preList.hashCode()
                        + " toString: "
                        + preList);
            }
            preList.beforeCompletion();
            lastIndexProcessed = lastIndexProcessed + 1;
        }

        // Do the same for the dbcp syncs
        lastIndexProcessed = 0;
        while ((lastIndexProcessed < postSyncs.size())) {
            Synchronization syncs = postSyncs.get(lastIndexProcessed);
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.postSyncs.before_completion - Class: " + syncs.getClass() + " HashCode: "
                        + syncs.hashCode()
                        + " toString: "
                        + syncs);
            }
            syncs.beforeCompletion();
            lastIndexProcessed = lastIndexProcessed + 1;
        }
    }
    ...
    public void afterCompletion(int status) {
        ...
        for (int i = preSyncs.size() - 1; i>= 0; --i) {
            Synchronization firstSyncs = preSyncs.get(i);
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.preSyncs.afterCompletion - Class: " + firstSyncs.getClass() + " HashCode: "
                        + firstSyncs.hashCode()
                        + " toString: " + firstSyncs);
            }
            try {
                firstSyncs.afterCompletion(status);
            } catch (Exception e) {
                ...
                log.warn("The pre-dbcp synchronization %s associated with tx %s failed during after completion", firstSyncs, tx, e);
            }
        }
        for (int i = postSyncs.size() - 1; i>= 0; --i) {
            Synchronization postSyncs = this.postSyncs.get(i);
            if (log.isTraceEnabled()) {
                log.trace("DBCPOrderedLastSynchronizationList.postSyncs.afterCompletion - Class: " + postSyncs.getClass() + " HashCode: "
                        + postSyncs.hashCode()
                        + " toString: "
                        + postSyncs);
            }
            try {
                postSyncs.afterCompletion(status);
            } catch (Exception e) {
                // Trap these exceptions so the rest of the synchronizations get the chance to complete
                // https://github.com/jbosstm/narayana/blob/5.0.4.Final/ArjunaJTA/jta/classes/com/arjuna/ats/internal/jta/resources/arjunacore/SynchronizationImple.java#L102
                log.warn("The DBCP synchronization %s associated with tx %s failed during after completion", postSyncs, tx, e);
            }
        }
        ...
        if (dbcpOrderedLastSynchronizations.remove(tx) == null) {
            ...
            Iterator<Entry<Transaction, DBCPOrderedLastSynchronizationList>> iterator = dbcpOrderedLastSynchronizations.entrySet().iterator();
            while (altKey == null && iterator.hasNext()) {
                Entry<Transaction, DBCPOrderedLastSynchronizationList> next = iterator.next();
                if (next.getValue().equals(this)) {
                    altKey = next.getKey();
                    iterator.remove();
                    if (log.isTraceEnabled()) {
                        log.trace("Removed: %s [%s]", System.identityHashCode(tx), tx.toString());
                    }
                    break;
                }
            }
